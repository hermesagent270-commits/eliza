...
export const CALENDAR_PLAN_INSTRUCTIONS = [
  ...
  "For update_event and delete_event the window describes when the TARGET event is now, never the time it should move to. A move or reschedule request usually names only the destination: then leave timeMin, timeMax, and windowLabel null so the wide lookup finds the current event, and put only the target's own words in queries.",
  ...
  'Example move (request: move my piano lesson to thursday at 6pm): {"subaction":"update_event","shouldAct":true,"response":null,"queries":["piano lesson"],"title":null,"tripLocation":null,"timeMin":null,"timeMax":null,"windowLabel":null}',
